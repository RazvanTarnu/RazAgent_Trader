# -*- coding: utf-8 -*-
"""KuCoin exchange adapter — normalized platform interface."""
...
logger = logging.getLogger("platform.exchange.kucoin")


class KuCoinAdapter(ExchangeProvider):
    """KuCoin spot adapter via ccxt."""
    ...
    def __init__(
        ...
        *,
        ...
        self._has_credentials = bool(api_key and api_secret and passphrase)
        if not self._has_credentials:
            self._paper_mode = True
            logger.info("KuCoinAdapter: missing credentials — operating in paper mode")

        self._client = ccxt.kucoin({
            "apiKey": api_key,
            "secret": api_secret,
            "password": passphrase,
    ...
    async def get_balances(self) -> list[Balance]:
        ...
        return parse_balances(self.name, items)
    ...
    async def place_order(self, request: OrderRequest) -> OrderResult:
        ...
        if not self._has_credentials:
            return OrderResult(success=False, exchange=self.name, error="Missing KuCoin credentials")
    ...
    async def test_connection(self) -> bool:
        try:
            ...
            logger.debug("KuCoin connection test failed: %s", safe_exception_message(exc))
            return False
