...
def test_backtest_no_future_leakage():
    ...
    engine = BacktestEngine(min_bars=30, min_trades=1)
...
def test_walk_forward_minimum_folds():
    bars = _make_bars(200)
    wf = WalkForwardEvaluator(train_size=60, test_size=20, step_size=20, min_folds=2)
    result = wf.evaluate("BTC/USDT", bars)
    assert len(result.folds) >= 2


def test_walk_forward_detects_insufficient_data():
    wf = WalkForwardEvaluator(train_size=120, test_size=30)
